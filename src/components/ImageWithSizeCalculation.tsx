...
function ImageWithSizeCalculation({url, altText, style, onMeasure, onLoadFailure, isAuthTokenRequired, objectPosition = CONST.IMAGE_OBJECT_POSITION.INITIAL}: ImageWithSizeCalculationProps) {
    ...
    console.debug(`@51888 ImageWitrhSize spinner status ${isLoading && !isImageCached && !isOffline} isLoading ${isLoading} !isImageCached ${!isImageCached} !isOffline ${!isOffline}`);
    return (
        <View style={[styles.w100, styles.h100, style]}>
            <Image
                ...
                onLoadStart={() => {
                    if (isLoadedRef.current ?? isLoading) {
                        console.debug(`@51888 ImageWitrhSize is not showing spinner`);
                        return;
                    }
                    console.debug(`@51888 ImageWitrhSize should show spinner`);
                    setIsLoading(true);
                }}
                onError={onError}
                onLoad={imageLoadedSuccessfully}
                waitForSession={() => {
                    console.debug(`@51888 ImageWitrhSize waitForSession`);
